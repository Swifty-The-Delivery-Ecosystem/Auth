...
const { ADMIN_NOT_FOUND_ERR, ACCESS_DENIED_ERR } = require("../errors");
...
exports.vendorApplicationStatusView = async (req, res, next) => {
  try {
    const newVendors = await Vendor.find({ status: "in process" }).select(
      "_id restaurantName ownerName location phone"
    );
    res.status(201).json({
      type: "success",
      data: {
